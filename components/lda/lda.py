...
def tfid_vectorizer(
    preproc_texts: pd.DataFrame, ngram_range: tuple = (1, 1)
) -> pd.DataFrame:
    vectorizer = TfidfVectorizer(ngram_range=ngram_range)

    vectorized_texts = vectorizer.fit_transform(preproc_texts)
    vectorized_texts = pd.DataFrame(
        vectorized_texts.toarray(), columns=vectorizer.get_feature_names_out()
    )
...
def print_topics(lda_model, vectorizer, top_words):
    ...
    for topic in range(n_components):
        print("-" * 10)
        print(
            f"For topic {topic}, here are the the top {top_words} words with weights:"
        )

        topic_df = (
            topic_mixture.iloc[topic].sort_values(ascending=False).head(top_words)
        )

        rich.print(round(topic_df, 3))
